...
class CliRouter
{
    ...
    public function __construct()
    {
        ...
        $this->addRoute('delete-spam-organisations', function () {
            ...
            $executor = (new \DSI\Repository\UserRepository())->getById(1);
            foreach ($spamIDs AS $spamID) {
                try {
                    $command = new \DSI\UseCase\Organisations\RemoveOrganisation();
                    $command->data()->executor = $executor;
                    $command->data()->organisation = (new \DSI\Repository\OrganisationRepository())
                        ->getById($spamID);
                    $command->exec();
        ...
        $this->addRoute('delete-spam-projects', function () {
            ...
            foreach ($spamIDs AS $spamID) {
                try {
                    ...
                    $command->data()->executor = $executor;
                    $command->data()->project = (new \DSI\Repository\ProjectRepository())
                        ->getById($spamID);
                    $command->exec();
