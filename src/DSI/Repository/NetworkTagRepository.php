...
class NetworkTagRepository
{
    private $table = 'network-tags';

    public function saveAsNew(NetworkTag $tag)
    {
        if (!$tag->getName())
    ...
    public function save(NetworkTag $tag)
    {
        if (!$tag->getName())
            throw new DSI\NotEnoughData('name');
        if ($this->nameExists($tag->getName()))
    ...
    public function remove(NetworkTag $tag)
    {
        $query = new SQL("SELECT id FROM `{$this->table}` WHERE id = '{$tag->getId()}' LIMIT 1");
        $existingTag = $query->fetch();
        if (!$existingTag)
    ...
    public function getById(int $id): NetworkTag
    {
    ...
    public function getByName(string $name): NetworkTag
    {
        return $this->getTagWhere([
            "`tag` = '" . addslashes($name) . "'"
        ]);
